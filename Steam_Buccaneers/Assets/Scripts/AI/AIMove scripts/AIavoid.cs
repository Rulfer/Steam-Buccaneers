...
public class AIavoid : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		relativePlayerPoint = transform.InverseTransformPoint(player.transform.position); //Used to check if the player is to the left or right of the enemy
	...
	private void sensors()
	{
		...
		{
			if(objectHit.transform.tag == "Planet" || objectHit.transform.tag == "Moon" || objectHit.transform.tag == "asteroid" || objectHit.transform.root.name == "Cargo(Clone)" || objectHit.transform.root.name == "Marine(Clone)" || objectHit.transform.root.name == "Boss(Clone)") //The planet is in front of the enemy
			{
				if(relativePlayerPoint.x > 0) //Player to the right of the enemy
				{
					this.GetComponent<AImove>().turnLeft = false;
					this.GetComponent<AImove>().turnRight = true;
				}
				else if(relativePlayerPoint.x <= 0)//Player to the left of the enemy
				{
			...
			else if(objectHit.transform.tag == "shopWall") //A shopWall is in front of the enemy
			{
				if(relativePlayerPoint.x > 0) //Player to the right of the enemy
				{
					this.GetComponent<AImove>().turnLeft = true;
