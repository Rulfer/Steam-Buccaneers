...
//		playerNewPos = player.transform.position; //Updates new player position
		...
		Debug.DrawRay(this.transform.position, fwd * 30, Color.green);
		Debug.DrawRay(this.transform.position, right * 30, Color.red);
		Debug.DrawRay(this.transform.position, left * 30, Color.blue);
...
//		}
	...
	private void planetSensors()
	{
		...
		if(Physics.Raycast(this.transform.position, fwd, out objectHit, 30))
		{
			if(objectHit.transform.tag == "Planet") //The planet is in front of the AI
			{
				if(relativePlanetPoint.x > 0) //planet to the right
				{
					Debug.Log("I hit the planet, dork");
					AImove.turnLeft = true;
					AImove.turnRight = false;
				}
				else if(relativePlanetPoint.x <= 0)
				{
		...
		if(Physics.Raycast(this.transform.position, left, out objectHit, 30))
		{
		...
		if(Physics.Raycast(this.transform.position, right, out objectHit, 30))
		{
